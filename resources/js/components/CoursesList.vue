        ...
        <div class="mt-3 text-center">
          ...
          <div class="mt-2 px-7 py-3">
            <p class="text-sm text-gray-500">
              ¿Estás seguro de que quieres eliminar el curso "{{ courseToDelete?.name }}"? Esta acción no se puede deshacer.
            </p>
          </div>
  ...
  methods: {
    async fetchCourses() {
      try {
        ...
        if (response.data.success) {
          ...
          this.error = response.data.message || 'Error al cargar los cursos';
        }
      } catch (error) {
        console.error('Error fetching courses:', error);
        this.error = error.response?.data?.message || 'Error de conexión al servidor';
      } finally {
        this.loading = false;
      }
    },
    ...
    async confirmDelete() {
      ...
      try {
        ...
        const response = await this.$http.delete(`/api/courses/${this.courseToDelete.id}`);
